...
export async function fetchCertInfo(entry_id: number): Promise<CertInfo | null> {
    ...
    const textEnd = d.indexOf(PATTERN_TEXT_END, textStart + PATTERN_TEXT_START.length);
    const text = d.slice(textStart + PATTERN_TEXT_START.length, textEnd)
        .replace(PATTERN_WHITESPACES,
            (matched) => {
                if (matched === "&nbsp;") {
                    return " ";
                }
